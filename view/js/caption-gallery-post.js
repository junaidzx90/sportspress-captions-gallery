jQuery(function ($) {
    ...
    $(".spg_caption_popup_content").on("submit", function(e){
        e.preventDefault();

        $(this).ajaxSubmit({
            url: caption_gallery_ajax.ajaxurl,
            data: {
                action: "store_caption_gallery_image",
            },
            dataType: "json",
            beforeSend: function(){
                $("#add_new_caption_glry_image").prop("disabled", true);
                $('.sub_button').find("svg").show();
            },
            success: function (response) {
                $("#add_new_caption_glry_image").removeAttr("disabled");
